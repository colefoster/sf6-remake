...
for (const entry of listCharacters() as unknown[]) {
  ...
  const idleLeg = idle.hips.y - (idle.feet[0]?.y ?? 0);
  ...
  for (const action of geo.actions) {
    ...
    for (let f = 1; f <= end; f++) {
      ...
      const legs = p.feet.length ? p.hips.y - p.feet[0]!.y : idleLeg;
      const stature = Math.abs((p.head?.y ?? p.neck.y) - (p.feet[0]?.y ?? p.hips.y));
      ...
      if (p.feet.some((foot) => foot.y > p.hips.y + 1)) flag("foot-above-hips", at, "");
      if (p.feet.length && legs > idleLeg * 2.2) flag("legs-stretched", at, `leg=${legs.toFixed(0)}`);
