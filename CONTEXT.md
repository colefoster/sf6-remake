...
- **Hit count** — how many times a move connects: distinct `HitID` per contiguous active window, summed (`hitCount`). Keys sharing an id can only connect once between them, so a blow split across three boxes is one hit; ids differing inside one window are the back-to-back hits FAT writes `1*3`. Counting keys instead is what wrongly called 397 single-hit moves multi-hit. See `docs/adr/0024-a-hit-is-a-hitid-not-a-key.md`.
...
- **Armor** — a hurtbox that absorbs a hit instead of taking it, for a stretch of frames. Applied **per hurtbox**, which is why body-only armor loses to a low attack: the leg box never had armor on it. It runs out: the atemi index the hurtbox carries is a **name** for the armor, and FAT's published hit count resolves it on 29 of 29 claims — index 1 is Drive Impact and is two hits, 3 and 7 are one. The atemi table itself is not in the dump, so three of its rows are known and the rest are not. See `docs/adr/0016-armor-is-per-hurtbox.md` and `docs/adr/0039-the-atemi-index-is-the-armors-name-and-fat-holds-the-payload.md`.
...
- **Command / motion** — what to *press*, from the trigger's own `commands` entry: an ordered list of steps, each a numpad direction or a **wildcard** (any direction it does not forbid). A group holds several accepted inputs, which is the game's input leniency written down — Ryu's Hadoken takes `236`, `4236` and `4136`. The game's dragon punch is `626`, not the `623` FAT publishes. Buttons are a bitmask union, so OD is literally all three punches. Charge moves name a slot and never say which way it is held; the direction is **inferred** from the release. See `docs/adr/0025-what-to-press-and-what-a-hit-does-to-you.md`.
- **Reaction** — which `DMG_*` (hit) or `GRD_*` (block) action the defender is put into, selected by the hit-data row's `strength` / `part` / `kind`. Fifteen of each per fighter, and the link between "a hit landed" and "the defender is in a real state".
...
- **Match** (`src/game/match.ts`) — two fighters on one clock: contact from box overlap, outcome from the hit table, reaction from that row, stun, hitstop, knockback, health. Its advantage on a blocked move is asserted to equal the scenario player's — two derivations, neither reading a published number.
- **Live projectile** — a fireball the match plays as a third body: spawned on the frame `ShotKey` names, on its own clock, travelling on its own motion, carrying its own hit data, and destroyed by an opposing fireball it meets. Nothing else can clear one — its hurtboxes are in the dump and unread. See `docs/adr/0029-the-match-throws-fireballs-and-two-of-them-meet.md`.
- **Stage half-width** — 765 game units from centre to either wall, and the **only number in the runtime that is in neither dump**. Borrowed from the FGC's own measurement, usable because that dataset's walk speeds and dash distances are identical to the dump's (Ryu 4.70 / 3.20 / 125.208 / 92.3). Nothing here can grade it. See `docs/adr/0030-the-stage-is-borrowed-and-the-corner-is-derived.md`.
- **Corner** — not the wall but what the wall refuses: a cornered fighter has no room to give, so pushbox separation and knockback both transfer into the *other* fighter. Derived from the half-width, not tuned on top of it. The wall stops the pushbox, so a cornered Ryu's origin rests 33 units short of it.
- **Gauge** — Drive and Super Art, in units where 10000 is one bar. Every trigger carries its own price (`focus_consume`, `gauge_consume`), so an unaffordable move is simply not an option and the same input reaches the non-OD version. Super's maximum is the dump's (30000); **Drive's 60000 is inferred** from OD costing two bars and is stated nowhere. See `docs/adr/0031-the-gauges-are-priced-by-the-dump-and-graded-by-fat.md`.
- **Drive drain** — the Drive a defender loses, which is *not* on the hit and block rows: those carry 0 and a positive gain. It is authored on the punish-counter and driveHit rows, where it matches FAT's `DDoH`/`DDoB` at 96% and 97%. Condition 3 is genuinely the punish counter (its hitstun delta matches FAT's `onPC` on 268/275 moves) and carries the ordinary hit's drain anyway.
- **Burnout** — Drive at zero. The `_tired` action twins and the type-47 branches that reach them are in the dump; the duration and the exit are not, so "leave when the gauge refills" is this project's rule, not the game's.
- **Round clock** — held in frames, displayed in counts at an assumed 60 frames per count, stopped by hitstop. A round ends on KO or on time, and a health tie on time is a draw.
- **Combo** — a run of hits the defender never acted between, carrying a hit count, damage, a juggle counter and a scaling percentage. Contact is keyed on `<side>:<action instance>:<HitID>`, so a multi-hit move lands each of its HitIDs; a hit the juggle rules refuse is not spent. See `docs/adr/0032-a-combo-is-a-hitid-a-counter-and-one-scaling-number.md`.
- **Juggle counter** — `Juggle1st` / `JuggleAdd` / `JuggleLimit`, all graded against FAT at 95–97%. The numbers are the dump's; **the rule they feed is asserted**, like the movement table. Read off the airborne row when the defender is airborne, because the values genuinely differ by defender state.
- **Starter scaling** — `fab.Combo._StartScaling`, FAT's `dmgScaling` "20% Start", agreeing on 196/200. It is the *only* scaling in the dump: SF6's per-hit curve and its minimum-scaling floor are in neither source, so a long combo here does close to full damage.
- **Combo Trials** — not reachable. MMDK dumps per-fighter `PlayerData` only, and trials live in mission assets; every combo claim here is per-move against FAT, never per-sequence against the game.
- **Knockdown** — `DmgType != 3`, graded against FAT's own "KD" at 92.8%. A **hard** knockdown is `_no_rolling` or `DownTime == 0` — neither says it alone, their union reads FAT's `HKD` at 96.1%. See `docs/adr/0033-dmgtype-is-the-knockdown-and-the-floor-time-is-not-recoverable.md`.
- **Floor time** — `DownTime`. The evidence is the counter-hit sweep: Ryu's 2HK states 10 on hit and 25 on counter with hitstun and knockback identical, so the only thing a counter changes is how long the defender lies there. The chain `DMG_*_DN → BAS_DN_STD_AO → up` carries **no branches in the dump** and is walked by name, like the jump chain.
- **Knockdown advantage** — *not reconstructible.* FAT publishes it on 784 moves; no reading of the dump's fields reproduced it (best 16%). The sim reports `defenderActionable: null` rather than inventing one.
- **Throw** — an ordinary box overlap against the defender's *throwable* box, unblockable and grounded opponents only. FAT's `throwRange × 100` equals the dump's throw-box reach on **24/24** characters — a second, independent confirmation of the unit system the stage half-width was borrowed with. See `docs/adr/0035-the-throwable-box-was-the-head-box.md`.
- **Throwable box** — resolves against the **pushbox base table** (`rects[7]`), not the hurtbox table its key sits beside. Read against `rects[8]` it silently returns the *head* box, which is what made throws look like a range check in ADR-0034. Correctly resolved it matches FAT's `throwHurt` on 23/24 with no offset.
- **Catch vs follow-through** — `NGS` catches for 0 damage and branches type-36 (`CATCH`) into `NGA_6`/`NGA_4`; the defender enters the paired action, the thrower's id **plus one**. The damage rides a **`LockKey`** (`_IsAttackDataHash02`, `Param02` = the hit-data row) — which is why those rows are referenced by no hitbox anywhere.
- **Drive Rush cancel** — cancelling a normal into the rush **discards the move's own recovery**; what the attacker waits out instead is the rush action's `freeze`, the same `WorldKey` timer ADR-0019 decoded for the super cinematic. 10 for the three-bar `DriveDash`, 11 for the half-bar `ParryDash`, on all 24 — read off the *trigger*, since which action is which is swapped on four fighters. The wait runs from the frame the rush window *opens*, not from contact: a cancellable normal carries that window as two consecutive live keys, and the earlier one is gated by `_Other` bit 17 and is not the one FAT times from. Reading the ungated key takes the grade from 64% to 95% on block and 95% on hit. See `docs/adr/0036-a-drive-rush-cancel-spends-the-rushs-freeze-not-the-moves-recovery.md` and `docs/adr/0038-the-rush-window-opens-late-and-a-gated-twin-hid-it.md`.
- **Absorbed hit** — armor eating a hit: health and hitstop still apply, the reaction does not. Checked against the **part** the attack landed on, since armor is per hurtbox — Drive Impact covers head, body and leg, so a low does not go under it. Broken by a Super Art or a Drive Reversal (`breaksArmor`, ADR-0017's rule, now shared between runtime and grader). See `docs/adr/0037-armor-absorbs-and-the-boxes-that-connected-are-the-ones-that-matter.md`.
- **Action instance** — a counter bumped every time a fighter enters an action. It is what makes a hitbox connect once: hitstop is longer than the active window, so any time-based rule re-hits. See `docs/adr/0027-two-fighters-and-the-reaction-the-table-asks-for.md`.
