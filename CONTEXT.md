...
- **Drive Rush cancel** — cancelling a normal into the rush **discards the move's own recovery**; what the attacker waits out instead is the rush action's `freeze`, the same `WorldKey` timer ADR-0019 decoded for the super cinematic. 10 for the three-bar `DriveDash`, 11 for the half-bar `ParryDash`, on all 24 — read off the *trigger*, since which action is which is swapped on four fighters. Graded at 64%, with the residual one or two frames. See `docs/adr/0036-a-drive-rush-cancel-spends-the-rushs-freeze-not-the-moves-recovery.md`.
