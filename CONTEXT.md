...
- **Throw** — a *range* check, not a box intersection: the throw hitbox spans y 0–130 and the throwable box sits at y 132–166, so the two are built never to meet and only their x-extents matter. Unblockable, grounded opponents only. FAT's `throwRange × 100` equals the dump's throw-box reach on **24/24** characters — a second, independent confirmation of the unit system [[stage half-width]] borrowed. See `docs/adr/0034-a-throw-is-a-range-check-and-its-damage-is-somewhere-else.md`.
- **Catch vs follow-through** — `NGS` is the catch (0 damage, 10 stun) and branches type-36 into `NGA_6`/`NGA_4`, which carries no hit keys. A throw's damage lives in a hit-data row referenced by **no key at all**; the rows are identifiable, the link from throw to row is not.
